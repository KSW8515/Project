...
void send_msg(MSG_INFO * msg_info, CLIENT_INFO * first_client_info)
{
	...
	else if(!strcmp(msg_info->to,"KICK"))
	{
		msg_info->msg[strlen(msg_info->msg) - 1] = '\0';
		...
		for(i=0;i<MAX_CLNT;i++)
		{
			if((first_client_info+i)->fd != -1)	
			{
				if (!strcmp(msg_info->msg, (first_client_info+i)->id))
				{
					char logbuf[BUF_SIZE];

                // 로그
                sprintf(logbuf, "KICK ID:%s (ip:%s, fd:%d)\n",
                        (first_client_info+i)->id,
                        (first_client_info+i)->ip,
                        (first_client_info+i)->fd);
                log_file(logbuf);

                // 연결 종료
                shutdown((first_client_info+i)->fd, SHUT_RDWR);
                close((first_client_info+i)->fd);

                // 서버 내부 상태 정리
                pthread_mutex_lock(&mutx);
                (first_client_info+i)->fd = -1;
                clnt_cnt--;
                pthread_mutex_unlock(&mutx);
