...
private:
    void load_camera_params(std::string yaml_path) {
        ...
        if (fs.isOpened()) {
            ...
            RCLCPP_WARN(this->get_logger(), "YAML not found! Using default values.");
    ...
    void timer_callback() {
        ...
        if (!cap_->read(frame)) return;
        	...
        	float markerLength = 0.105;  //10.5cm
		...
		if (!ids.empty()) {
		    ...
		    for (size_t i = 0; i < ids.size(); ++i) {
		        ...
		        if (i == 0) {
		            twist_msg.angular.z = -0.3 * x;
		            twist_msg.linear.x = (distance > 0.2) ? 0.03 : 0.0;
		        }
		
		        RCLCPP_INFO(this->get_logger(),
		                    "X: %.3f Y: %.3f Z: %.3f Dist: %.3f",
		                    x, y, z, distance);
		    }
