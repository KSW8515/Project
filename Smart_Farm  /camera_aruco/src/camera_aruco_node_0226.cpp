...
public:
    enum class State { IDLE, GO_TO_WAFFLE, MOVING, RETURN_1, RETURN_2, RETURN_3, WAIT };
    ...
    CameraArucoNode() : Node("camera_aruco_node"), state_(State::IDLE) {
		...
		burger_ready_pub_ = this->create_publisher<std_msgs::msg::Bool>("/burger_response", 10);
        ...
        amcl_sub_ = this->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
            "/amcl_pose", 10, std::bind(&CameraArucoNode::amcl_callback, this, std::placeholders::_1));

		waffle_ready_sub_ = this->create_subscription<std_msgs::msg::Bool>(
    	"/burger_check", 10, std::bind(&CameraArucoNode::waffle_check_callback, this, std::placeholders::_1));
...
private:
	...
	void waffle_check_callback(const std_msgs::msg::Bool::SharedPtr msg) {
	    if (msg->data) {
	        auto response = std_msgs::msg::Bool();
	        
	        if (state_ == State::MOVING && current_marker_dist_ < 0.2) {
	            response.data = true;
	            RCLCPP_INFO(this->get_logger(), "Burger is READY! Sending true.");
	        } else {
	            response.data = false;
	            RCLCPP_WARN(this->get_logger(), "Burger NOT ready. Sending false. (Dist: %.2f)", current_marker_dist_);
	        }
	        burger_ready_pub_->publish(response);
	    }
	}
    ...
    rclcpp::Publisher<std_msgs::msg::Bool>::SharedPtr burger_ready_pub_;
    
	rclcpp::Subscription<std_msgs::msg::String>::SharedPtr signal_sub_;
    rclcpp::Subscription<std_msgs::msg::Bool>::SharedPtr waffle_ready_sub_;
